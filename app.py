...
def create_main_trend_chart(df, param_to_show):
    ...
    fig.update_layout(
        title={'text': "PARAMETER TREND ANALYSIS", 'y':0.9, 'x':0.5, 'xanchor': 'center', 'yanchor': 'top'},
    ...
    if data.empty:
        ...
        with main_col2:
            with st.container():
                ...
                st.plotly_chart(create_main_trend_chart(data, params_to_plot), use_container_width=True)
                st.markdown('</div>', unsafe_allow_html=True)
        ...
        for i, p in enumerate(STATUS_THRESHOLDS.keys()):
            with gauge_cols[i]:
                 with st.container():
                    st.markdown('<div class="hud-container">', unsafe_allow_html=True)
                    st.plotly_chart(create_meter_gauge(latest[p], p), use_container_width=True)
                    st.markdown('</div>', unsafe_allow_html=True)


elif app_mode == "Data Explorer":
    st.subheader("Explore Raw Sensor Data")
    # ... (Database explorer code remains the same)
